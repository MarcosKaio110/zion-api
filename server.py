...
# A MÁGICA: O Funil SHA-256 que limpa a senha invisível
def pre_hash(password: str):
    return hashlib.sha256(password.encode('utf-8', errors='ignore')).hexdigest()

def hash_pass(password: str):
    return pwd_context.hash(pre_hash(password))

def verify_pass(plain, hashed):
    return pwd_context.verify(pre_hash(plain), hashed)
...
def login(user: UserAuth):
    ...
    return {"wallet_hash": db_user["wallet_hash"], "username": db_user["username"], "saldo": float(db_user["saldo"])}
...
def transferir_sng(req: TransferRequest):
    ...
    if not rem.data or float(rem.data[0]["saldo"]) < req.quantidade: raise HTTPException(status_code=400, detail="Saldo insuficiente.")
    target_hash = dest_res.data[0]["wallet_hash"] if (dest_res := supabase.table("usuarios").select("wallet_hash").eq("username", req.destinatario.lower()).execute() if req.destinatario.startswith("@") else req.destinatario).data else None
    if not target_hash: raise HTTPException(status_code=404, detail="Destinatário não existe.")
